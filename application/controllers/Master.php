...
class Master extends CI_Controller
{
  ...
  public function employee()
  {
    ...
    $dtemployee = $this->db->order_by('id', 'DESC')->get('employee')->result_array(); 
    if ( !empty($contract) && !empty( $time ) ) {

      if ( $contract == "m" ){

        $this->db->select('*')->from('employee')
                  ->where('hire_end_date !=', null)
                  ->where('TIMESTAMPDIFF(MONTH, hire_date, hire_end_date) = ', $time);
        $dtemployee = $this->db->get()->result_array();
      }
    }

    // Employee Data
    $d['title'] = 'Karyawan';
    $d['employee'] = $dtemployee;
    $d['account'] = $this->Admin_model->getAdmin($this->session->userdata['username']);
  ...
  public function a_employee()
  {
    ...
    $this->form_validation->set_rules('e_hire_end_date', 'End Hire Date', 'required|trim');
  ...
  private function _addEmployee()
  {
    ...
    $end_hire_date = $this->input->post('e_hire_end_date');
  ...
  public function e_location($l_id)
  {
    ...
    // Form Validation
    $this->form_validation->set_rules('l_name', 'Location Name', 'required');
    ...
    if ($this->form_validation->run() == false) {
      ...
      $data['longitude']  = $this->input->post('longitude'); 
      $data['range']      = $this->input->post('range'); 
      $this->_editLocation($l_id, $data);
    }
  }
  private function _editLocation($l_id, $data)
  {
